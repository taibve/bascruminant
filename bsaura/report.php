...
if (isset($_POST["import"])) {
    if (isset($_FILES["csv_upload"])) {
        $csvFileName = $_FILES["csv_upload"]["name"];
        $csvFileTmpName = $_FILES["csv_upload"]["tmp_name"];

        // Get the uploaded file's name and create a unique name
        $csvFilePath = $csvFolder . $csvFileName;

        // Move the uploaded file to the target folder
        if (move_uploaded_file($csvFileTmpName, $csvFilePath)) {
            // Insert information into the database
            $sqlInsert = "INSERT INTO csv_tbl (id, csv) VALUES (?, ?)";
            $stmt = $connection->prepare($sqlInsert);
            $stmt->bind_param("ss", $csvid, $csvFilePath);

            if ($stmt->execute()) {
                $successMessage = "CSV File and Information Imported into the database";

                // Redirect to the same page to refresh
                header("Location: report.php");
                exit(); // Make sure to exit after the header to prevent further execution
            } else {
                $errorMessage = "Problem in importing CSV file and information: " . $stmt->error;
            }

            $stmt->close();
        } else {
            $errorMessage = "Error moving the CSV file.";
        }
    } else {
...
<head>
  <meta charset="UTF-8">
  <title>BIOLOGICAL ASSET REPORT</title>
  <!--style-->
...
<div class="layout has-sidebar fixed-sidebar fixed-header">
      ...
      <div class="layout" style="background-color:#e6e6fa;">
        ...
        <div class="form-group row">
            <label class="col-sm-3 col-form-label form-label">CSV File</label>
            <div class="col-sm-6">
                <input type="file" class="form-control" name="csv_upload" accept=".csv">
            </div>
        </div>

        <div class="form-group row">
            <div class="offset-sm-3 col-sm-3">
                <button type="submit" name="import" class="btn btn-import"><i class="fa-solid fa-file-import"></i>&nbsp;&nbsp;Import</button>
            </div>
        </div>
    </form>
    ...
    // Display CSV file contents if available
    if (!empty($csvFiles)) {
        echo "<h2>CSV Files in the Folder</h2>";
        echo "<table class='table table-striped'>";
        echo "<thead class='thead-dark'><tr><th>File Name</th><th>Actions</th></tr></thead>";
        echo "<tbody>";
        foreach ($csvFiles as $file) {
            ...
            echo "<td>$filename</td>";
            echo "<td>
            <a href='vreport.php?file=$filename' class='btn btn-primary'><i class='fa-solid fa-eye'></i>&nbsp;View</a>
            <a href='preport.php?file=$filename' target='_blank' class='btn btn-success'><i class='fa-solid fa-print'></i>&nbsp;Print</a>
                  </td>";
            echo "</tr>";
        }
        echo "</tbody></table>";
  ...
  <script src='https://unpkg.com/@popperjs/core@2'></script><script src="template.js"></script>
