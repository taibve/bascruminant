...
function printTableScript() {
    echo '<script>
            function printTable() {
                ...
                // Exclude the last column header (Actions) from printing
                ...
                // Exclude the last column (Actions) from printing in each data row
                var dataRows = printableTable.getElementsByTagName("tr");
                for (var i = 0; i < dataRows.length; i++) {
                    var cells = dataRows[i].getElementsByTagName("td");
                    if (cells.length > 0) {
                        dataRows[i].removeChild(cells[cells.length - 1]); // Remove the last cell (Actions)
                    }
                }
...
<div class="container">
    ...
    <a class="btn btn-primary" href="cmedical.php" role="button"><i class="fa-solid fa-file"></i>&nbsp;New Medical Information</a>
    <div>
        ...
        <table id="printableTable" class="table table-striped">
            <thead>
                <tr>
                    <th>Animal ID</th>
                    <th>Deworming</th>
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        if (isset($_POST['selectedAnimal']) && $_POST['selectedAnimal'] !== 'all') {
                            ...
                            $animalType = strtolower($row['animal_type']); // Get animal type from the database
                        ...
                        echo "<td>" . htmlspecialchars($row['deworming']) . "</td>";
                        ...
                        echo "<td>";
                        echo "<a class='btn btn-primary btn-sm' href='emedical.php?id=" . htmlspecialchars($row['animal_id']) . "'><i class='fa-solid fa-pen-to-square'></i></a><br>";
                        echo "</td>";
                        echo "</tr>";
                    }
                } else {
