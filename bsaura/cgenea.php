...
if ($_SERVER["REQUEST_METHOD"] == 'POST') {
    ...
    do {
        if (empty($animalid) || empty($father_id) || empty($mother_id) || empty($category)) {
        ...
        $sql = "INSERT INTO genea_tbl (animal_id, father_id, mother_id, category) VALUES ( ?, ?, ?, ?)";
        $stmt = $connection->prepare($sql);
        $stmt->bind_param("ssss", $animalid, $father_id, $mother_id, $category);
        $result = $stmt->execute();
        ...
        header("location: genealogical.php");
        exit;
    } while (false);
}
?>
...
<div class="layout has-sidebar fixed-sidebar fixed-header">
    ...
    <div class="container my 5">
        ...
        <form action="" method="post" enctype="multipart/form-data">
          <div class="row mb-3">
            <label class="col-sm-3 col-form-label" for="animal_id">Animal ID:</label>
            <div class="col-sm-6">
              <select name="animal_id" id="animal_id" class="form-control">
              <option disabled selected>ANIMAL ID</option>
                  <?php
                    // Fetch animal_ids from animal_tbl
                    $animalIdsQuery = "SELECT animal_id FROM animal_tbl";
                    $animalIdsResult = $connection->query($animalIdsQuery);
                    if ($animalIdsResult->num_rows > 0) {
                        while ($row = $animalIdsResult->fetch_assoc()) {
                            $selected = ($row['animal_id'] == $animalid) ? 'selected' : '';
                            echo "<option value='{$row['animal_id']}' $selected>{$row['animal_id']}</option>";
                        }
                    }
                    ?>
                </select>
                ...
                <div class="row mb-3">
                  <label class="col-sm-3 col-form-label" for="father_id">Father ID:</label>
                  <div class="col-sm-6">
                      <select name="father_id" id="father_id" class="form-control">
                      <option disabled selected>FATHER ID</option>
                          <option>NO PARENT</option>
                          <?php
                          // Fetch only female animal_ids from animal_tbl
                          $animalIdsQuery = "SELECT animal_id FROM animal_tbl WHERE sex = 'Male'";
                          $animalIdsResult = $connection->query($animalIdsQuery);
                          ...
                          if ($animalIdsResult->num_rows > 0) {
                              ...
                              echo "<option disabled>No female animals found</option>";
                          }
                          ?>
                      </select>
                  </div>
            ...
            <div class="row mb-3">
                <label class="col-sm-3 col-form-label" for="mother_id">Mother ID:</label>
                <div class="col-sm-6">
                    <select name="mother_id" id="mother_id" class="form-control">
                        <option disabled selected>MOTHER ID</option>
                        <option>NO PARENT</option>
                        <?php
                        // Fetch only female animal_ids from animal_tbl
                        $animalIdsQuery = "SELECT animal_id FROM animal_tbl WHERE sex = 'Female'";
                        $animalIdsResult = $connection->query($animalIdsQuery);
            ...
            <div class="row mb-3">
                <label class="col-sm-3 col-form-label">Animal Type: </label>
                  <div class="col-sm-6">
                    <select id="text" name="category" type="text" class="form-control" value="<?php echo $animaltype ?>">
                ...
                <div class="col-sm-3 d-grid">
                    ...
                    <script>
                        document.getElementById("cancelButton").addEventListener("click", function () {
                            ...
                            if (confirmCancel) {
                                window.location.href = "genealogical.php";
                            }
                        });
                    </script>
                </div>
